...
colormap = cm.get_cmap("cmr.sunburst")

background = 'black'
coefs = [-2.232124051075165,1.2278205615944087,2.4835607501779187,0.4179458263081921]
...
cvs = ds.Canvas(plot_width = int(sizepx), plot_height = int(sizepx), x_range=(-5.5,5.5), y_range=(-5.5,5.5))
agg = cvs.points(df, 'x', 'y')
img = tf.set_background(tf.shade(agg, cmap = colormap),background)
buf = io.BytesIO()
img.to_pil().save(buf, format='PNG')
image = Image.open(buf)
image.save("attractor201.png","PNG")
buf.close()
image.close()
img.close()
print('DONE')
